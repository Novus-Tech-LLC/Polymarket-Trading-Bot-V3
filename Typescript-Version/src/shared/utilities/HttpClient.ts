...
import { NetworkError, normalizeError } from './ErrorHandler';
import { RETRY_CONFIG, TimeConstants } from './ApplicationConstants';
...
export class HttpClient {
     ...
     */
    static async fetch<T = unknown>(url: string): Promise<T> {
        ...
        for (let attempt = 1; attempt <= retries; attempt++) {
            try {
                ...
                if (isNetworkError(error) && !isLastAttempt) {
                    ...
                    console.warn(
                        `⚠️  Network error (attempt ${attempt}/${retries}), retrying in ${delay / TimeConstants.SECOND_MS}s...`
                    );
                    await sleep(delay);
                    continue;
                }
