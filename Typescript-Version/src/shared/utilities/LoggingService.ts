...
import { LOG_CONSTANTS } from './ApplicationConstants';

class LoggingService {
    private static logsDir = path.join(process.cwd(), LOG_CONSTANTS.LOGS_DIR);
    private static currentLogFile = '';

    private static getLogFileName(): string {
        const date = new Date().toISOString().split('T')[0]; // YYYY-MM-DD
        return path.join(this.logsDir, `${LOG_CONSTANTS.LOG_FILE_PREFIX}${date}.log`);
    }
    ...
    static header(title: string) {
        console.log('\n' + chalk.cyan('━'.repeat(LOG_CONSTANTS.MAX_LINE_LENGTH)));
        console.log(chalk.cyan.bold(`  ${title}`));
        console.log(chalk.cyan('━'.repeat(LOG_CONSTANTS.MAX_LINE_LENGTH)) + '\n');
    ...
    static trade(traderAddress: string, action: string, details: any) {
        console.log('\n' + chalk.magenta('─'.repeat(LOG_CONSTANTS.MAX_LINE_LENGTH)));
        ...
        console.log(chalk.magenta('─'.repeat(LOG_CONSTANTS.MAX_LINE_LENGTH)) + '\n');
    ...
    static separator() {
        console.log(chalk.dim('─'.repeat(LOG_CONSTANTS.MAX_LINE_LENGTH)));
    }
